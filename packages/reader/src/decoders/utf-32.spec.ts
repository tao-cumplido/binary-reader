...
test('valid', ({ deepEqual }) => {
	...
	deepEqual(utf32.decode({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian }), {
		value: String.fromCodePoint(0xd7ff),
		source: buffer.subarray(0, 4),
	});
	deepEqual(utf32.decode({ buffer, offset: 4, byteOrder: ByteOrder.BigEndian }), {
		value: String.fromCodePoint(0xe000),
		source: buffer.subarray(4, 8),
	});
	deepEqual(utf32.decode({ buffer, offset: 8, byteOrder: ByteOrder.BigEndian }), {
...
test('invalid', ({ deepEqual, throws }) => {
...

	{
		...
		deepEqual(
			throws(() => utf32.decode({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian })),
