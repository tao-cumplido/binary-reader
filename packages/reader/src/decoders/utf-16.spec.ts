...
test('valid', ({ deepEqual }) => {
	...
	deepEqual(utf16.decode({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian }), {
		value: String.fromCodePoint(0xd7ff),
		source: buffer.subarray(0, 2),
	});

	deepEqual(utf16.decode({ buffer, offset: 2, byteOrder: ByteOrder.BigEndian }), {
		value: String.fromCodePoint(0xe000),
		source: buffer.subarray(2, 4),
	});

	deepEqual(utf16.decode({ buffer, offset: 4, byteOrder: ByteOrder.BigEndian }), {
		value: String.fromCodePoint(0x10000),
		source: buffer.subarray(4, 8),
	});

	deepEqual(utf16.decode({ buffer, offset: 8, byteOrder: ByteOrder.BigEndian }), {
...
test('invalid', ({ deepEqual, throws }) => {
...

	{
		...
		deepEqual(
			throws(() => utf16.decode({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian })),
