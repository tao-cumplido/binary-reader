...
test.describe("bigint", () => {
	...
	test("mixed endianness", () => {
		...
		assert.deepEqual(read({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian, }), {
			value: 0x1234n,
			source: buffer.subarray(0, 2),
		});

		assert.deepEqual(read({ buffer, offset: 2, byteOrder: ByteOrder.LittleEndian, }), {
			value: 0x7856n,
	...
	test("no byte order", () => {
		...
		assert.deepEqual(bigintReader({ signed: false, byteLength: 1, })({ buffer, offset: 0, }), {
			value: 0n,
			source: buffer.subarray(0, 1),
		});

		assert.throws(
			() => bigintReader({ signed: false, byteLength: 2, })({ buffer, offset: 0, }),
	...
	test("incomplete buffer", () => {
		...
		assert.throws(
			() => bigintReader({ signed: false, byteLength: 2, })({ buffer, offset: 0, byteOrder: ByteOrder.BigEndian, }),
